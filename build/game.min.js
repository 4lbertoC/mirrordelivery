/* http://github.com/4lbertoC/mirrordelivery */var ja={};
...
e.L=db;e.k=0;e.v=0;e.position=f[r.Z].slice();a=f[r.A].slice();y=[f[r.A][0]+15,f[r.A][1]+23];n=y.slice();v=1;K.position=K.h=null;d.time=0;Q(s.g);N=0;ba="4444444444444444444444444444444444444444"+f[r.aa]+"4444444444444444444444444444444444444444";ba=ba.split("");Aa=null;Ba(f);ua.length=0;ra.length=0;G.clearRect(0,0,I,H);G.fillStyle="rgba(0,0,0,0.7)";G.fillRect(0,0,I,H);G.fillStyle="#fff";G.textAlign="center";G.fillText("A 13kB game by Alberto Congiu",I/2,12);G.font="20px courier";G.fillText(d.m[r.Q],
I/2,H/2-50);G.fillText("Luke "+d.da+" - "+d.ea+" Crow",I/2,H/2);G.font="15px courier";G.fillText("Click the "+(Z?"screen":"crow")+" to start",I/2,H/2+60);G.fillText("< >: Select level",I/2,H/2+100);G.fillText(f[r.P]?"E: EDIT   "+(Z?"S":"D")+": DELETE   "+(Z?"A":"J")+": EXPORT   I: IMPORT":"E: EDIT   I: IMPORT",I/2,H/2+120);G.beginPath();G.arc(e.position[0],e.position[1],16,0,360);G.lineWidth=2;G.strokeStyle="#fff";G.stroke();Na=pa.slice();window.localStorage&&(window.localStorage.pa=JSON.stringify(C))}
function Q(f){d.state=f;if(Z){var b=document.getElementsByClassName("edit");f="btn edit"+(f===s.e?"":" h");for(var a=0;a<b.length;a++)b[a].setAttribute("class",f)}}function va(){Q(s.i);d.time=Ia+d.m[r.G];Ea&&h();U.style.cursor="none"}function P(){d.da+=1;U.style.cursor="auto";A(D.sa);E(x)}function ta(){d.ea+=1;U.style.cursor="auto";A(D.la);E(x)}function ca(f,b){return Math.round(f[0]/w)===Math.round(b[0]/w)&&Math.round(f[1]/w)===Math.round(b[1]/w)}function R(b,a){return Math.atan((a[1]-b[1])/(a[0]-
b[0]))}function da(b,a){return b&&a&&Math.sqrt(Math.pow(b[0]-a[0],2)+Math.pow(b[1]-a[1],2))}function T(b,a){return ba[$*Math.floor(a/w)+Math.floor(b/w)]}function M(b,a,c,d,e){var g=a,n=T(b,g);c=b+c;topRight=T(c,g);bottomRight=T(c,a+d);bottomLeft=T(b,a);return n===e||topRight===e||bottomRight===e||bottomLeft===e}function ea(b,a,c,d,e){a+=d;d=T(b,a);bottomLeft=T(b+c,a);return d===e||bottomLeft===e}function fa(){U.style.cursor="auto";d.state===s.e&&(C[x][r.aa]=ba.join("").substring($,$*La-$),C[x][r.ba]=
b.position.slice(),C[x][r.Z]=e.position.slice(),C[x][r.A]=a.slice(),C[x][r.W]=Pa.O.slice());E(x)}function Da(){var a=b.S,c=la();b.c||(a=0);a=Math.min(c,a+c/7);b.S=a;return ga(B.o)?c/2:a}function la(){return Math.max(1,b.speed+b.U-(void 0!==b.d?J[b.d].size:0))}function ga(a){return M(b.position[0]-8,b.position[1]-32,16,32,a)||M(b.position[0],b.position[1]-8,0,0,a)}function wa(a){a.preventDefault();if(d.state===s.e)Ya=!Ya;else if(d.state===s.i&&e.v<Ia)if(a=l(a),(ea(e.position[0]-8,e.position[1]-8,16,
16,B.t)||M(e.position[0],e.position[1],0,0,B.t))&&e.k<Va)A(D.Y),e.k++;else{var b;for(b=0;b<ua.length;b++)if(ca(a,ua[b].position)){e.k=Math.max(e.k,Fa);A(D.Y);ua.splice(b,1);break}}}function V(){if(d.state===s.e)if(N<ha){var f=$*Math.floor(z[1]/w)+Math.floor(z[0]/w);f<ba.length&&(ba[f]=""+N)}else if(N===X.qa)b.position=z.slice();else if(N===X.s)e.position=z.slice();else if(N===X.w)a=z.slice(),y=[a[0]+15,a[1]+23],n=y.slice();else{if(N===X.l)for(Pa.O=z.slice(),f=0;f<J.length;f++){var c=J[f];c.position[0]=
z[0]+16*f;c.position[1]=z[1]-c.size}}else d.state!==s.g||Z?0<e.k&&e.v<Ia&&(e.k--,A(D.ka),f=Object.create(lb),f.position=e.position.slice(),ra.push(f)):z[0]>e.position[0]-16&&(z[0]<e.position[0]+16&&z[1]>e.position[1]-16&&z[1]<e.position[1]+16)&&va()}function Ka(a){e.v=a+mb;A(D.na);e.k=0;e.L--;0>=e.L?P():e.L<eb&&ma(a)}function ma(a){if(!Aa){Aa=[];for(var b=0;b<ba.length;b++)ba[b]===B.D&&Aa.push(b)}b=Aa[Math.floor(Math.random()*Aa.length)]+1;b=[b%$*w,Math.floor(b/$)*w];K.h=[b[0]-6,b[1]-10];K.position||
(K.position=K.h.slice());K.wa=a+nb;v+=0.05}function na(a,c){A(D.ra);b.d=void 0;a.position=a.O.slice();b.I--;0>=b.I?ta():b.I<eb&&ma(c)}function xa(a){a=a||C[x];a=["CustomLevel"+ob++,a[r.ba].slice(),a[r.Z].slice(),a[r.W].slice(),a[r.A].slice(),a[r.G],a[r.aa],a[r.l].slice(),null,!0];C.push(a);d.state===s.g&&(x=C.length-1,E(x))}function oa(a,b,c){var d=null,e=K.position?"#c29":"#00deff";a===B.D?d=F.D:a===B.F?d=F.F:a===B.Da?e="#000":a===B.ma?e="#0f0":a===B.t?d=F.t:a===B.o?d=F.o:a===B.n&&(d=F.n);t.fillStyle=
e;t.fillRect(b,c,w,w);null!==d&&g(d,b,c,w,w)}function ya(a){if(a.position&&a.h){var b=R(a.position,a.h),c=a.position[0]>a.h[0]?-1:1,d=Math.abs(a.position[0]-a.h[0]),e=Math.abs(a.position[1]-a.h[1]);if(3<d||3<e){var g=16*Math.cos(b)*c,b=16*Math.sin(b)*c;Math.abs(g)>d&&(g=d*g/Math.abs(g));Math.abs(b)>e&&(b=e*b/Math.abs(b));a.position[0]+=g;a.position[1]+=b}}}var I=640,H=400,w=16,$=I/w,La=H/w,F={s:0,Ba:1,n:2,w:3,o:4,C:5,t:6,F:7,Ca:8,D:9,ya:10,za:11},W=[{frame:{x:30,y:0,a:30,b:14},f:{x:1,y:1,a:30,b:14},
frames:[{x:1,y:1,a:15,b:11},{x:17,y:1,a:15,b:13}]},{frame:{x:14,y:40,a:14,b:8},f:{x:1,y:7,a:14,b:8}},{frame:{x:32,y:14,a:16,b:16},f:{x:0,y:0,a:16,b:16}},{frame:{x:0,y:24,a:14,b:24},f:{x:1,y:7,a:14,b:24}},{frame:{x:16,y:48,a:16,b:16},f:{x:0,y:0,a:16,b:16}},{frame:{x:0,y:0,a:30,b:24},f:{x:1,y:3,a:30,b:24},frames:[{x:0,y:0,a:15,b:24},{x:16,y:0,a:13,b:24}]},{frame:{x:48,y:14,a:16,b:10},f:{x:0,y:10,a:16,b:10}},{frame:{x:14,y:24,a:16,b:16},f:{x:0,y:0,a:16,b:16}},{frame:{x:31,y:32,a:6,b:14},f:{x:4,y:1,a:6,
b:14}},{frame:{x:0,y:48,a:16,b:16},f:{x:0,y:0,a:16,b:16}},{frame:{x:36,y:32,a:11,b:13},f:{x:0,y:0,a:12,b:13}},{frame:{x:49,y:25,a:15,b:16},f:{x:0,y:0,a:15,b:16}}],aa=[523.25,587.33,659.26,698.46,783.99,880,987.77,1046.5,1108.73],Ea=!(!window.btoa||!window.atob),pa="023123467".split(""),D={oa:0,ra:1,ka:2,Aa:3,X:4,ja:5,sa:6,la:7,ia:8,Y:9,n:10,na:11},s={g:0,i:1,e:2},B={EMPTY:"0",D:"1",o:"2",F:"3",Da:"4",ma:"5",t:"6",n:"7"},ha=8,X={qa:ha+0,s:ha+1,w:ha+2,l:ha+3},za=ha+4,Va=3,Fa=5,mb=5E3,db=10,eb=4,nb=
2E3,cb=10,Qa=12,fb=[1,-6],r={Q:0,ba:1,Z:2,W:3,A:4,G:5,aa:6,l:7,B:8,P:9},U=document.getElementById("C"),t=U.getContext("2d"),qa=new Image,Ma=document.createElement("canvas"),ab=0,Ra=document.createElement("canvas"),G=Ra.getContext("2d"),Ga=[],bb={},Na=null,z=[0,0],Sa=!1,Y={ga:{},N:function(a){Y.ga[a.keyCode]=void 0},M:function(a){Y.ga[a.keyCode]=Ia}},Z=!1,Ja=document.getElementById("o"),ia=Ja.getBoundingClientRect(),gb=[100,100],Za=!1,sa=null,Ha=null,Oa=null,hb=null,Ia=0,d={state:s.g,R:U.getBoundingClientRect(),
time:0,da:0,ea:0,m:null},ba=[],Aa=null,N=0,Ya=!1,b={position:null,speed:4,S:0,U:0,ha:0,Ja:2,Ia:-7,u:!1,j:!1,c:!1,q:!1,r:0,d:void 0,H:0,I:cb,p:!1},e={position:null,h:null,k:0,v:0,L:db};n=y=a=null;v=1;var K={position:null,h:null,wa:0},Pa={fa:null,size:1,position:null,O:[0,0]},J=[],pb={position:null},ua=[],lb={position:null,speed:6},ra=[],ib=[["Tutorial",[33,370],[I-24,55],[300,365],[20,70],3E6,"",
